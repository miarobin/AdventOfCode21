...
def collBasin(array):
    grid = [[10 for _ in range(len(array[0])+2)]] + [[10] + line + [10] for line in array] + [[10 for _ in range(len(array[0])+2)]]
    basins = [len(basin(grid, set(), i, j))  for i in range(1,len(grid)-1) for j in range(1,len(grid[0])-1) if all(grid[i][j]<grid[a][b] for a, b in ((i+1, j), (i, j+1), (i-1, j), (i, j-1)))]
